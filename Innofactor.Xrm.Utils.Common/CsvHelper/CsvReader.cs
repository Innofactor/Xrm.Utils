...
namespace ElencySolutions.CsvHelper
{
    ...
    public sealed class CsvReader : IDisposable
    {
        ...
        public bool RemoveEmptyTrailingFields = false;
            ...
            if (line.Length > 0)
            {
                ...
                for (var i = 0; i < line.Length; i++)
                {
                    ...
                    if (inQuotes)
                    {
                        ...
                        if (character == '"' && ((line.Length > (i + 1) && line[i + 1] == Delimeter) || ((i + 1) == line.Length)))
                        {
                            inQuotes = false;
                            inColumn = false;
                            i++;
                ...
                if (inColumn || line[line.Length - 1] == Delimeter)
                {
                    Fields.Add(TrimColumns ? _columnBuilder.ToString().Trim() : _columnBuilder.ToString());
                }

                if (RemoveEmptyTrailingFields)
                {
                    while (Fields.Count > 0 && string.IsNullOrEmpty(Fields[Fields.Count - 1]))
                    {
